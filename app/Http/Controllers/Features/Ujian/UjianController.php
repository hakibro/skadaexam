...
class UjianController extends Controller
{
    ...
    public function exam(Request $request, $jadwal_id = null)
    {
        ...
        if ($jadwalId) {
            ...
            if ($jadwalUjian && $enrollment->status_enrollment !== 'active') {
                ...
                $enrollment->waktu_selesai_ujian = $enrollment->waktu_selesai_ujian
                    ?? $enrollment->waktu_mulai_ujian->copy()->addMinutes(
                        $jadwalUjian->durasi_menit ?? 0
                    );
                $enrollment->save();
            }
        } else {
    ...
    public function recordViolation(Request $request)
    {
        ...
        // Pastikan hasil ujian milik siswa ini
        $hasilUjian = HasilUjian::where('id', $request->hasil_ujian_id)
            ->where('siswa_id', $siswa->id)
            ->firstOrFail();
        ...
        if (!$recentViolation) {
            ...
            PelanggaranUjian::create([
                ...
                'deskripsi' => 'Pelanggaran: ' . $request->reason,
