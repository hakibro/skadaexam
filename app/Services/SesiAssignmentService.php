...
class SesiAssignmentService
{
    ...
    private function createNewSesiByDuplication(JadwalUjian $jadwalUjian, string $targetDate): int
    {
        ...
        foreach ($activeRooms as $room) {
            ...
            // Jika ruangan ini belum punya sesi template, skip
            if ($templateSesis->isEmpty()) {
                continue;
            }
    ...
    private function createBasicSesiForAllRooms(JadwalUjian $jadwalUjian, string $targetDate): int
    {
        ...
        foreach ($activeRooms as $room) {
            ...
            // Tidak ada assignment siswa yang bisa dicopy pada basic sesi, skip assignment siswa
    ...
    /**
     * Auto-enroll siswa from assigned sesi ruangan to jadwal ujian 
     * based on kelas and jurusan compatibility
     */
    private function autoEnrollStudentsFromAssignedSesi(JadwalUjian $jadwalUjian): int
    {
        $enrolledCount = 0;
        ...
        foreach ($assignedSesi as $sesi) {
            ...
            foreach ($sesi->sesiRuanganSiswa as $sesiSiswa) {
                ...
                // Check if student is eligible for this jadwal ujian based on kelas and jurusan
                if (!$this->isSiswaEligibleForJadwal($siswa, $jadwalUjian)) {
                    Log::debug("Siswa {$siswa->nama} (ID: {$siswa->id}) not eligible for jadwal {$jadwalUjian->kode_ujian} - kelas/jurusan mismatch");
                    continue;
                }

                // Check if student is already enrolled in this jadwal ujian
                $existingEnrollment = \App\Models\EnrollmentUjian::where('siswa_id', $siswa->id)
                    ->where('jadwal_ujian_id', $jadwalUjian->id)
                    ->first();

                if ($existingEnrollment) {
                    Log::debug("Siswa {$siswa->nama} (ID: {$siswa->id}) already enrolled in jadwal {$jadwalUjian->kode_ujian}");
                    continue;
                }

                // Create enrollment
                try {
                    \App\Models\EnrollmentUjian::create([
                        'siswa_id' => $siswa->id,
                        'jadwal_ujian_id' => $jadwalUjian->id,
                        'sesi_ruangan_id' => $sesi->id,
                        'status_enrollment' => 'enrolled',
                        'catatan' => 'Auto-enrolled from sesi assignment'
                    ]);

                    $enrolledCount++;

                    Log::info("Auto-enrolled siswa {$siswa->nama} (ID: {$siswa->id}) to jadwal {$jadwalUjian->kode_ujian} in sesi {$sesi->nama_sesi}");
                } catch (\Exception $e) {
                    Log::error("Failed to auto-enroll siswa {$siswa->nama} (ID: {$siswa->id}) to jadwal {$jadwalUjian->kode_ujian}: " . $e->getMessage());
                }
            }
        }
    ...
    /**
     * Check if siswa is eligible for jadwal ujian based on kelas_target and jurusan compatibility
     */
