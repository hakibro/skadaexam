...
@section('content')
    <div class="space-y-6">
        <form action="{{ route('naskah.sesi.update', [$jadwal->id, $sesi->id]) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="bg-white shadow-md rounded-lg overflow-hidden">
                <div class="p-4 sm:p-6 border-b">
                    <h3 class="text-lg font-medium text-gray-900">Form Edit Sesi Ujian</h3>
                    <p class="mt-1 text-sm text-gray-600">Edit sesi ujian untuk jadwal "{{ $jadwal->nama_ujian }}" pada
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="jenis_sesi" id="jenis_sesi" required
                                ...
                                <option value="reguler"
                                    {{ old('jenis_sesi', $sesi->jenis_sesi) == 'reguler' ? 'selected' : '' }}>Reguler
                                </option>
                                <option value="susulan"
                                    {{ old('jenis_sesi', $sesi->jenis_sesi) == 'susulan' ? 'selected' : '' }}>Susulan
                                </option>
                            </select>
                            @error('jenis_sesi')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="time" name="waktu_mulai" id="waktu_mulai" required
                                    value="{{ old('waktu_mulai', $sesi->waktu_mulai->format('H:i')) }}"
                            ...
                            <div>
                                ...
                                <input type="time" name="waktu_selesai" id="waktu_selesai" required
                                    value="{{ old('waktu_selesai', $sesi->waktu_selesai->format('H:i')) }}"
                        ...
                        <div>
                            ...
                            <input type="number" name="kapasitas_maksimal" id="kapasitas_maksimal" required min="1"
                                value="{{ old('kapasitas_maksimal', $sesi->kapasitas_maksimal) }}"
                            ...
                            @error('kapasitas_maksimal')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                            @if ($sesi->peserta_terdaftar > 0)
                                <p class="mt-1 text-xs text-amber-600">
                                    <i class="fa-solid fa-exclamation-triangle mr-1"></i>
                                    Sudah ada {{ $sesi->peserta_terdaftar }} peserta terdaftar di sesi ini
                                </p>
                            @endif
                        </div>
                            ...
                            <select name="ruangan_id" id="ruangan_id"
                                ...
                                @foreach ($ruangans as $ruangan)
                                    <option value="{{ $ruangan->id }}"
                                        {{ old('ruangan_id', $sesi->ruangan_id) == $ruangan->id ? 'selected' : '' }}>
                                        {{ $ruangan->nama_ruangan }} ({{ $ruangan->kode_ruangan }}) - Kapasitas:
                                        {{ $ruangan->kapasitas }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="pengawas_id" id="pengawas_id"
                                ...
                                @foreach ($pengawas as $guru)
                                    <option value="{{ $guru->id }}"
                                        {{ old('pengawas_id', $sesi->pengawas_id) == $guru->id ? 'selected' : '' }}>
                                        {{ $guru->nama }}
                                    </option>
                                @endforeach
                            </select>
