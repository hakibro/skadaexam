...
@section('content')
    <div class="max-w-4xl mx-auto">
        <form action="{{ route('naskah.soal.store') }}" method="POST" enctype="multipart/form-data" id="soal-form">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white shadow rounded-lg p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <select name="bank_soal_id" id="bank_soal_id" required
                                ...
                                @foreach ($bankSoals as $bank)
                                    <option value="{{ $bank->id }}"
                                        ...
                                        {{ old('bank_soal_id', $selectedBankSoal?->id) == $bank->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <input type="number" name="nomor_soal" value="{{ old('nomor_soal', $nextNomorSoal) }}"
                                min="1" required
                                class="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-blue-500 focus:border-blue-500">
                            @error('nomor_soal')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <select name="tipe_soal" id="tipe_soal" required
                                ...
                                @foreach (\App\Models\Soal::QUESTION_TYPES as $value => $label)
                                    <option value="{{ $value }}"
                                        {{ old('tipe_soal', $selectedBankSoal?->tipe_soal_default ?? 'pilihan_ganda') == $value ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="bg-white shadow rounded-lg p-6">
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            <label
                                ...
                                <input type="radio" name="tipe_pertanyaan" value="teks"
                                    {{ old('tipe_pertanyaan', 'teks') == 'teks' ? 'checked' : '' }}
                            ...
                            <label
                                ...
                                <input type="radio" name="tipe_pertanyaan" value="gambar"
                                    {{ old('tipe_pertanyaan') == 'gambar' ? 'checked' : '' }} class="mr-3 text-blue-600">
                                <div>
                            ...
                            <label
                                ...
                                <input type="radio" name="tipe_pertanyaan" value="teks_gambar"
                                    {{ old('tipe_pertanyaan') == 'teks_gambar' ? 'checked' : '' }}
                                    class="mr-3 text-blue-600">
                                <div>
                    ...
                    <div id="pertanyaan-teks-section">
                        ...
                        <textarea name="pertanyaan" rows="4" placeholder="Masukkan pertanyaan di sini..."
                            class="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-blue-500 focus:border-blue-500">{{ old('pertanyaan') }}</textarea>
                        @error('pertanyaan')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <!-- Pertanyaan Gambar -->
                    <div id="pertanyaan-gambar-section" class="mt-6 hidden">
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                        ...
                        <div
                            ...
                            <div id="gambar-pertanyaan-preview" class="mt-4 hidden">
                                <img id="gambar-pertanyaan-img" src="" alt="Preview"
                ...
                <div id="listening-audio-section" class="bg-white shadow rounded-lg p-6 hidden">
                ...
                <div id="pilihan-jawaban-section" class="bg-white shadow rounded-lg p-6">
                    ...
                    @foreach (['A' => 'a', 'B' => 'b', 'C' => 'c', 'D' => 'd', 'E' => 'e'] as $label => $value)
                        <div class="mb-6 p-4 border border-gray-200 rounded-lg pilihan-option" data-option="{{ $label }}">
                            ...
                            <!-- Pilihan Teks -->
                            <div class="pilihan-teks-section-{{ $value }}">
                                <textarea name="pilihan_{{ $value }}_teks" rows="2"
                                    placeholder="Masukkan pilihan {{ $label }}..."
                                    class="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-blue-500 focus:border-blue-500">{{ old("pilihan_{$value}_teks") }}</textarea>
                            ...
                            <!-- Pilihan Gambar -->
                            <div class="pilihan-gambar-section-{{ $value }} hidden">
                                <div class="border-2 border-dashed border-gray-200 rounded-lg p-4 text-center">
                                    ...
                                    <div id="pilihan-{{ $value }}-preview" class="mt-2 hidden">
                                        <img id="pilihan-{{ $value }}-img" src="" alt="Preview"
                                            class="max-w-full max-h-32 mx-auto rounded shadow">
                                    </div>
                                </div>
                    ...
                    <div class="mt-6">
                        ...
                        <div class="flex space-x-4">
                            @foreach (['A', 'B', 'C', 'D', 'E'] as $kunci)
                                <label class="flex items-center">
                                    <input type="radio" name="kunci_jawaban_single" value="{{ $kunci }}" class="kunci-single mr-2 text-blue-600"
                                        {{ old('kunci_jawaban') == $kunci ? 'checked' : '' }}>
                                    <input type="checkbox" name="kunci_jawaban_multi[]" value="{{ $kunci }}" class="kunci-multi mr-2 text-blue-600 hidden"
                                        {{ in_array($kunci, old('kunci_jawaban_multi', [])) ? 'checked' : '' }}>
                                    <span class="font-medium">{{ $kunci }}</span>
                                </label>
                            @endforeach
                        </div>
                        <input type="hidden" name="kunci_jawaban" id="kunci_jawaban_hidden" value="{{ old('kunci_jawaban') }}">
                        @error('kunci_jawaban')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div id="kunci-teks-section" class="bg-white shadow rounded-lg p-6 hidden">
                    ...
                    <textarea id="kunci_jawaban_text" rows="2"
                        ...
                        class="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-blue-500 focus:border-blue-500">{{ old('kunci_jawaban') }}</textarea>
                ...
                <div id="interactive-section" class="bg-white shadow rounded-lg p-6 hidden">
                    ...
                    <div id="matching-editor" class="space-y-3 hidden">
                        @for ($i = 0; $i < 5; $i++)
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <input type="text" name="interactive_left[]" value="{{ old('interactive_left.' . $i) }}"
                                    placeholder="Item kiri {{ $i + 1 }}"
                                    class="border border-gray-300 rounded-md px-3 py-2 focus:ring-blue-500 focus:border-blue-500">
                                <input type="text" name="interactive_right[]" value="{{ old('interactive_right.' . $i) }}"
                    ...
                    <div id="ordering-editor" class="space-y-3 hidden">
                        @for ($i = 0; $i < 8; $i++)
                            <input type="text" name="interactive_items[]" value="{{ old('interactive_items.' . $i) }}"
                    ...
                    <div id="dragdrop-editor" class="grid grid-cols-1 md:grid-cols-2 gap-4 hidden">
                        <div class="space-y-3">
                            ...
                            @for ($i = 0; $i < 6; $i++)
                                <input type="text" name="interactive_drag_items[]" value="{{ old('interactive_drag_items.' . $i) }}"
                        ...
                        <div class="space-y-3">
                            ...
                            @for ($i = 0; $i < 6; $i++)
                                <input type="text" name="interactive_zones[]" value="{{ old('interactive_zones.' . $i) }}"
                ...
                <div class="bg-white shadow rounded-lg p-6">
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            <label
                                ...
                                <input type="radio" name="pembahasan_tipe" value="teks"
                                    {{ old('pembahasan_tipe', 'teks') == 'teks' ? 'checked' : '' }}
                                    class="mr-2 text-blue-600">
                            ...
                            <label
                                ...
                                <input type="radio" name="pembahasan_tipe" value="gambar"
                                    {{ old('pembahasan_tipe') == 'gambar' ? 'checked' : '' }} class="mr-2 text-blue-600">
                                <span class="text-sm">Gambar Saja</span>
                            </label>
                            <label
                                class="flex items-center p-3 border-2 border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50">
                                <input type="radio" name="pembahasan_tipe" value="teks_gambar"
                                    {{ old('pembahasan_tipe') == 'teks_gambar' ? 'checked' : '' }}
                                    class="mr-2 text-blue-600">
                    ...
                    <div id="pembahasan-teks-section">
                        ...
                        <textarea name="pembahasan_teks" rows="4" placeholder="Masukkan pembahasan soal di sini..."
                            class="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-blue-500 focus:border-blue-500">{{ old('pembahasan_teks') }}</textarea>
                    ...
                    <!-- Pembahasan Gambar -->
                    <div id="pembahasan-gambar-section" class="mt-6 hidden">
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                        ...
                        <div
                            ...
                            <div id="pembahasan-gambar-preview" class="mt-4 hidden">
                                <img id="pembahasan-gambar-img" src="" alt="Preview"
                                    class="max-w-full max-h-64 mx-auto rounded-lg shadow">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('bank_soal_id').addEventListener('change', handleBankSoalChange);
        ...
        function handleTipeSoalChange() {
            ...
            const singleInputs = document.querySelectorAll('.kunci-single');
            const multiInputs = document.querySelectorAll('.kunci-multi');
            ...
            singleInputs.forEach(input => input.classList.toggle('hidden', tipe === 'pilihan_kompleks'));
            multiInputs.forEach(input => input.classList.toggle('hidden', tipe !== 'pilihan_kompleks'));
